...
def main(args):
    ...
    for t in range(100):
        with torch.inference_mode():
            camera_names = ['rgb:left_cam:240x424:2d', 'rgb:right_cam:240x424:2d', 'rgb:top_cam:240x424:2d',
                            'rgb:Franka_wrist_cam:240x424:2d']
            # camera_names = ['rgb:right_cam:240x424:2d']
            image_dict = dict()
            for cam_name in camera_names:
                # image_dict[cam_name] = path_data[f'{cam_name}'][t]
                image_dict[cam_name] = np.random.rand(240, 424, 3)
            ...
            qpos_numpy = np.random.rand(6).astype(np.float32)
